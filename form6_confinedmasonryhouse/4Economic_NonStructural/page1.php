<!-- Confined Masonry House -->
<!-- Economic Loss-Inducing Factors related to the Non-Structural Elements -->

<form action="page1-backend.php">

<!-- 1 -->
<p>1. Acceleration and Displacement Hazards</p>

<p>1.1 Shelves</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_1-1[]" value="a"> Not anchored to structural system
<input type="checkbox" name="_1-1[]" value="b"> Contents not strapped

<p>1.2 Hangings from roof/floor and from walls</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_1-2[]" value="a"> Heavy weight and unstrapped

<p>1.3 Gas cylinders and geysers</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_1-3[]" value="a"> Unsecured to the wall

<p>1.4 Items on adjacent building</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_1-4[]" value="a"> Unsecured to the adjacent building

<!-- 2 -->
<p>2. Lifelines</p>

<p>2.1 Electric supply</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_2-1[]" value="a"> Cable is taught between house and pole

<p>2.2 Water supply</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_2-2[]" value="a"> Pipeline is rigidly held between house and mains

<p>2.3 Sewage</p>
<p>You may select 0 or more.</p>
<input type="checkbox" name="_2-3[]" value="a"> Sewage line is rigidly held between house and mains


<br>
<input type="submit" value="submit">
</form>

